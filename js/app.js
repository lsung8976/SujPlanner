...
// === 중국어 실용 단어/표현 (50개) - 글자별 훈음 포함 ===
{char:'人工智能',reading:'rén gōng zhì néng',meaning:'인공지능 (AI)',detail:'훈음: 人(사람 인) 工(장인 공) 智(슬기 지) 能(능할 능)\n예: 人工智能技术 (AI 기술)'},
{char:'深度学习',reading:'shēn dù xué xí',meaning:'딥러닝',detail:'훈음: 深(깊을 심) 度(법도 도) 学(배울 학) 习(익힐 습)\n예: 深度学习模型 (딥러닝 모델)'},
{char:'算法',reading:'suàn fǎ',meaning:'알고리즘',detail:'훈음: 算(셈할 산) 法(법 법)\n예: 优化算法 (최적화 알고리즘)'},
{char:'服务器',reading:'fú wù qì',meaning:'서버',detail:'훈음: 服(옷 복/섬길 복) 务(힘쓸 무) 器(그릇 기)\n예: 云服务器 (클라우드 서버)'},
{char:'分布式',reading:'fēn bù shì',meaning:'분산식',detail:'훈음: 分(나눌 분) 布(베 포) 式(법 식)\n예: 分布式系统 (분산 시스템)'},
{char:'推理',reading:'tuī lǐ',meaning:'추론/인퍼런스',detail:'훈음: 推(밀 추) 理(다스릴 리)\n예: 模型推理 (모델 인퍼런스)'},
{char:'训练',reading:'xùn liàn',meaning:'훈련/트레이닝',detail:'훈음: 训(가르칠 훈) 练(익힐 련)\n예: 训练数据 (훈련 데이터), 预训练 (프리트레이닝)'},
{char:'显卡',reading:'xiǎn kǎ',meaning:'그래픽카드/GPU',detail:'훈음: 显(나타날 현) 卡(막힐 카)\n예: 高端显卡 (고급 GPU)'},
{char:'内存',reading:'nèi cún',meaning:'메모리/RAM',detail:'훈음: 内(안 내) 存(있을 존)\n예: 内存不足 (메모리 부족)'},
{char:'缓存',reading:'huǎn cún',meaning:'캐시',detail:'훈음: 缓(느릴 완) 存(있을 존)\n예: KV缓存 (KV 캐시)'},
{char:'吞吐量',reading:'tūn tǔ liàng',meaning:'처리량/Throughput',detail:'훈음: 吞(삼킬 탄) 吐(토할 토) 量(헤아릴 량)\n예: 高吞吐量 (높은 처리량)'},
{char:'延迟',reading:'yán chí',meaning:'지연/Latency',detail:'훈음: 延(늘일 연) 迟(더딜 지)\n예: 低延迟 (낮은 지연)'},
{char:'论文',reading:'lùn wén',meaning:'논문',detail:'훈음: 论(논할 론) 文(글월 문)\n예: 发表论文 (논문 발표)'},
{char:'研究生',reading:'yán jiū shēng',meaning:'대학원생',detail:'훈음: 研(갈 연) 究(궁구할 구) 生(날 생)\n예: 硕士研究生 (석사과정)'},
{char:'导师',reading:'dǎo shī',meaning:'지도교수',detail:'훈음: 导(인도할 도) 师(스승 사)\n예: 博士导师 (박사 지도교수)'},
{char:'实验',reading:'shí yàn',meaning:'실험',detail:'훈음: 实(열매 실) 验(증험할 험)\n예: 做实验 (실험하다)'},
{char:'数据集',reading:'shù jù jí',meaning:'데이터셋',detail:'훈음: 数(셈 수) 据(의거할 거) 集(모을 집)\n예: 训练数据集 (훈련 데이터셋)'},
{char:'开源',reading:'kāi yuán',meaning:'오픈소스',detail:'훈음: 开(열 개) 源(근원 원)\n예: 开源代码 (오픈소스 코드)'},
{char:'部署',reading:'bù shǔ',meaning:'배포/디플로이',detail:'훈음: 部(거느릴 부) 署(관청 서)\n예: 模型部署 (모델 배포)'},
{char:'优化',reading:'yōu huà',meaning:'최적화',detail:'훈음: 优(넉넉할 우) 化(될 화)\n예: 性能优化 (성능 최적화)'},
{char:'并行',reading:'bìng xíng',meaning:'병렬',detail:'훈음: 并(나란히 병) 行(다닐 행)\n예: 并行计算 (병렬 연산)'},
{char:'量化',reading:'liàng huà',meaning:'양자화/Quantization',detail:'훈음: 量(헤아릴 량) 化(될 화)\n예: 模型量化 (모델 양자화)'},
{char:'调度',reading:'diào dù',meaning:'스케줄링',detail:'훈음: 调(고를 조) 度(법도 도)\n예: 任务调度 (태스크 스케줄링)'},
{char:'瓶颈',reading:'píng jǐng',meaning:'병목',detail:'훈음: 瓶(병 병) 颈(목 경)\n예: 性能瓶颈 (성능 병목)'},
{char:'权重',reading:'quán zhòng',meaning:'가중치/Weight',detail:'훈음: 权(권세 권) 重(무거울 중)\n예: 模型权重 (모델 가중치)'},
{char:'迁移',reading:'qiān yí',meaning:'마이그레이션/전이',detail:'훈음: 迁(옮길 천) 移(옮길 이)\n예: 迁移学习 (전이학습)'},
{char:'框架',reading:'kuàng jià',meaning:'프레임워크',detail:'훈음: 框(틀 광) 架(시렁 가)\n예: 深度学习框架 (딥러닝 프레임워크)'},
{char:'加速',reading:'jiā sù',meaning:'가속',detail:'훈음: 加(더할 가) 速(빠를 속)\n예: GPU加速 (GPU 가속)'},
{char:'异构',reading:'yì gòu',meaning:'이기종/Heterogeneous',detail:'훈음: 异(다를 이) 构(얽을 구)\n예: 异构计算 (이기종 컴퓨팅)'},
{char:'弹性',reading:'tán xìng',meaning:'탄력성/Elastic',detail:'훈음: 弹(탄알 탄) 性(성품 성)\n예: 弹性计算 (탄력적 컴퓨팅)'},
{char:'负载均衡',reading:'fù zài jūn héng',meaning:'로드밸런싱',detail:'훈음: 负(질 부) 载(실을 재) 均(고를 균) 衡(저울 형)\n예: 服务器负载均衡'},
{char:'预填充',reading:'yù tián chōng',meaning:'프리필(Prefill)',detail:'훈음: 预(미리 예) 填(메울 전) 充(채울 충)\nLLM 서빙의 Prefill 단계'},
{char:'解码',reading:'jiě mǎ',meaning:'디코딩(Decode)',detail:'훈음: 解(풀 해) 码(부호 마)\n예: 自回归解码 (Auto-regressive decoding)'},
{char:'注意力机制',reading:'zhù yì lì jī zhì',meaning:'어텐션 메커니즘',detail:'훈음: 注(부을 주) 意(뜻 의) 力(힘 력) 机(틀 기) 制(지을 제)\nTransformer의 핵심'},
{char:'大语言模型',reading:'dà yǔ yán mó xíng',meaning:'대규모 언어 모델(LLM)',detail:'훈음: 大(큰 대) 语(말씀 어) 言(말씀 언) 模(본뜰 모) 型(거푸집 형)\n예: 大语言模型推理'},
{char:'微调',reading:'wēi tiáo',meaning:'파인튜닝',detail:'훈음: 微(작을 미) 调(고를 조)\n예: 指令微调 (Instruction Fine-tuning)'},
{char:'提示词',reading:'tí shì cí',meaning:'프롬프트',detail:'훈음: 提(끌 제) 示(보일 시) 词(말씀 사)\n예: 提示词工程 (프롬프트 엔지니어링)'},
{char:'上下文',reading:'shàng xià wén',meaning:'컨텍스트',detail:'훈음: 上(위 상) 下(아래 하) 文(글월 문)\n예: 上下文窗口 (컨텍스트 윈도우)'},
{char:'向量',reading:'xiàng liàng',meaning:'벡터',detail:'훈음: 向(향할 향) 量(헤아릴 량)\n예: 词向量 (워드 벡터)'},
{char:'稀疏',reading:'xī shū',meaning:'스파스(Sparse)',detail:'훈음: 稀(드물 희) 疏(성길 소)\n예: 稀疏注意力 (Sparse Attention)'},
{char:'蒸馏',reading:'zhēng liù',meaning:'디스틸레이션/증류',detail:'훈음: 蒸(찔 증) 馏(증류할 류)\n예: 知识蒸馏 (Knowledge Distillation)'},
{char:'混合专家',reading:'hùn hé zhuān jiā',meaning:'MoE(Mixture of Experts)',detail:'훈음: 混(섞을 혼) 合(합할 합) 专(오로지 전) 家(집 가)\n예: 混合专家模型'},
{char:'端到端',reading:'duān dào duān',meaning:'End-to-End',detail:'훈음: 端(끝 단) 到(이를 도) 端(끝 단)\n예: 端到端训练 (E2E 트레이닝)'},
{char:'鲁棒性',reading:'lǔ bàng xìng',meaning:'로버스트니스/강건성',detail:'훈음: 鲁(노나라 로) 棒(몽둥이 봉) 性(성품 성)\nrobust 음역어. 예: 模型鲁棒性'},
{char:'收敛',reading:'shōu liǎn',meaning:'수렴/Convergence',detail:'훈음: 收(거둘 수) 敛(거둘 렴)\n예: 训练收敛 (학습 수렴)'},
{char:'过拟合',reading:'guò nǐ hé',meaning:'오버피팅',detail:'훈음: 过(지날 과) 拟(비길 의) 合(합할 합)\n예: 防止过拟合 (오버피팅 방지)'},
{char:'梯度',reading:'tī dù',meaning:'그래디언트',detail:'훈음: 梯(사다리 제) 度(법도 도)\n예: 梯度下降 (경사하강법)'},
{char:'嵌入',reading:'qiàn rù',meaning:'임베딩',detail:'훈음: 嵌(박을 감) 入(들 입)\n예: 词嵌入 (워드 임베딩)'},
{char:'基准测试',reading:'jī zhǔn cè shì',meaning:'벤치마크',detail:'훈음: 基(터 기) 准(준할 준) 测(헤아릴 측) 试(시험 시)\n예: 性能基准测试'},
{char:'可扩展性',reading:'kě kuò zhǎn xìng',meaning:'확장성/Scalability',detail:'훈음: 可(옳을 가) 扩(넓힐 확) 展(펼 전) 性(성품 성)\n예: 系统可扩展性'}
];
